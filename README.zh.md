...
Forma 是实验品。它今天没有效果系统、没有包获取（只有路径依赖）、没有 YAML/TOML 解析器、没有 trait、没有类型收窄。静态推断宁可显式说"不知道"，也不猜测。这些不是疏忽，是刻意：先把"类型即元数据"这一个假设验证到根，再谈扩张。64 份 RFC 记录了每一步的取舍——包括每个被拒绝的替代方案。
