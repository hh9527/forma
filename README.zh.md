...
JSON 和 TOML 文件可以像源码一样进入同一个不可变模块图。TOML table 变成普通 Dict，四种时间标量则通过 `@bim/std/toml.DateTime` 保留为经过校验、类别明确的 Tagged String。codec 失败时同时保留外部数据位置，以及施加约束的 Forma 类型声明位置。
...
Forma 是实验品。它今天没有效果系统、没有包获取（只有路径依赖）、没有 YAML 解析器、没有 trait、没有类型收窄。静态推断宁可显式说"不知道"，也不猜测。这些不是疏忽，是刻意：先把"类型即元数据"这一个假设验证到根，再谈扩张。65 份 RFC 记录了每一步的取舍——包括每个被拒绝的替代方案。
